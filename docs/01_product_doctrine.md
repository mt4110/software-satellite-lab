...
- human-gated learning-candidate inspection
- Satellite Evidence Pack による declarative evidence workflow 拡張
- thin UI + strong CLI
```
...
`software-satellite-lab` は「作業を忘れない flight recorder」を作る。
...
```text
A local-first AI Coding Flight Recorder for AI-assisted software work.
...
```text
AI 開発作業の flight recorder / evidence ledger。
