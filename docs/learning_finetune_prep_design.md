...
- typed lifecycle summary
  - `learning preview` の `review_queue[].lifecycle_summary.lifecycle_state` として追加済み
  - queued / running / blocked / paused / completed / failed / cancelled
  - `completed` は learning prep inspection 上の完了であり、training export / JSONL生成 / training job の完了ではない
  - `rejected` は明示的な cancel ではないため `blocked` として扱い、`cancelled` は cancelled / canceled / aborted 系の trace に限る
